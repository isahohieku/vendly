...
const UserCardModal = ({ isOpen, setModalOpen, user }: Props) => {
  return (
    <Modal
      ...
      actionBtnClassName="bg-[#E0FAF8] border border-[#00D0BE] w-full text-[#006E72] rounded-[5px] text[15px] font-[800] h-[45px]"
      btnnText="Continue"
    >
      <div className="flex">
        ...
        <div>
          <p className="font-semibold text-[15px] leading-6">
            {user?.name?.first} {user?.name?.last}
          </p>
          <p className="text-[#007575] font-normal leading-5 mt-1">
            @{user?.username || user?.name.last}
          </p>

          <p className="text-[#007575] font-[600] text-[10px] mt-[10px]">
            Following 1179 <span className="inline-block mx-3">|</span>333k Followers
          </p>
